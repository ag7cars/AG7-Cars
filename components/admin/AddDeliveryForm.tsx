...
export default function AddDeliveryForm() {
  ...
  function addFiles(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    const mismatched = selected.filter((file) => !file.type.startsWith("video/"));

    if (mismatched.length > 0) {
      setMessage("This page is for videos only — please add MP4/WebM/MOV files.");
      return;
    }
  ...
  const onSubmit: SubmitHandler<DeliveryFormValues> = async (values) => {
    ...
    try {
      ...
      formData.append("delivery", JSON.stringify({ ...values, mediaKind: "video" }));
      items.forEach((item) => {
      ...
      setMessage(
        totalCount > 1 ? `${totalCount} videos successfully published.` : "Video successfully published."
      );
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
      ...
      <section className="rounded-2xl border border-white/10 bg-white/[0.04] p-6">
        <h2 className="text-xl font-semibold">Delivery Videos</h2>

        <p className="mt-2 text-sm text-white/50">
          Add one or more videos — each becomes its own entry in AG7 Deliveries Videos, using the
          Brand and Model below. Up to 10 at a time. Uploaded files are stored directly on the
          server (up to 150 MB each).
        </p>
        ...
        >
          {items.length === 0 ? "Add Videos" : `Add More (${totalCount}/10)`}
        </button>
      ...
      <section className="rounded-2xl border border-white/10 bg-white/[0.04] p-6">
        ...
        <p className="mt-2 text-sm text-white/50">
          Shown on the landing page next to every video added above.
        </p>
